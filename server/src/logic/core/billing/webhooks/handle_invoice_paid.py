...
from server.src.app.config.email_templates import BillingPaymentReceivedTemplate
...
async def handle_invoice_paid(pool: Pool, cache: Redis, data_obj: dict) -> None:
    ...
    async with pool.acquire() as conn:
        ...
        await insert_invoice(
            ...
            status=data_obj.get("status", "paid"),
    ...
    amount_cents: int = data_obj.get("amount_paid", 0)
    currency: str = data_obj.get("currency", "usd").upper()
    timestamp = datetime.now(timezone.utc).strftime("%B %d, %Y at %H:%M UTC")
    template = BillingPaymentReceivedTemplate(
