...
from server.src.store.sql.core.api_keys.ui.get_project_api_keys import ProjectApiKeyRecord, get_project_api_keys
from server.src.store.sql.core.organizations.shared.resolve_user_role_in_org import resolve_user_role_in_org


async def get_project_api_keys_data(
    pool: Pool,
    org_role_cache: TTLCache,
    user_id: UUID,
    organization_id: UUID,
    project_id: UUID,
) -> list[ProjectApiKeyRecord]:
    ...
    async with pool.acquire() as conn:
        return await get_project_api_keys(conn, organization_id, project_id)
