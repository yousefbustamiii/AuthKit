...
async def complete_github_oauth(
    ...
    try:
        access_token = await exchange_github_code(http, code, state)
        user_profile = await get_github_user_profile(http, access_token)
        raw_email = user_profile["email"]
        name = user_profile.get("name")
        avatar_url = user_profile.get("avatar_url")
    except httpx.HTTPStatusError as exc:
        logger.warning("GitHub OAuth provider error: status=%s", exc.response.status_code)
        raise OAuthProviderError() from exc
    except ValueError as exc:
        logger.warning("GitHub OAuth no verified email: %s", exc)
        raise OAuthEmailNotVerifiedError() from exc
    except Exception as exc:
        logger.warning("GitHub OAuth unexpected error: %s", type(exc).__name__)
        raise OAuthProviderError() from exc
